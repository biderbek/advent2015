...
directions_max['^']=directions.count('^')
directions_max['v']=directions.count('v')
directions_max['<']=directions.count('<')
directions_max['>']=directions.count('>')

smatrix_dims['height']=directions_max['^']+directions_max['v']
smatrix_dims['width']=directions_max['>']+directions_max['<']

smatrix = [[0] * (smatrix_dims['width']) for i in xrange(smatrix_dims['height'])]

#set initial coordinates and first mark
x,y=directions_max['>'],directions_max['v']
smatrix[x][y]=1
santa=0
robo_x,robo_y=x,y
...
for move in directions:
    if santa==0:
        ...
        if move=='v':
            ...
            robo_x-=1
        santa=0
        smatrix[robo_x][robo_y]+=1

# Answer the question
locations['total'] = smatrix_dims['height']*smatrix_dims['width']
locations['unused'] = 0

for row in smatrix:
    locations['unused']+=row.count(0)

print "hi",locations['total']-locations['unused']
